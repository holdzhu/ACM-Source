#include <cstdio>
#include <cstring>
#include <vector>
#include <queue>
#include <algorithm>

using namespace std;

#define INF 1000000000
#define maxn 5000

struct Edge
{
	int from, to, cap, flow, cost;
	Edge(int u, int v, int c, int f, int w) : from(u), to(v), cap(c), flow(f), cost(w) {}
};

struct MCMF
{
	int n, m;
	vector<Edge> edges;
	vector<int> G[maxn];
	int inq[maxn];
	int d[maxn];
	int p[maxn];
	int a[maxn];

	void init(int n)
	{
		this->n = n;
		for (int i = 0; i < n; ++i)
		{
			G[i].clear();
		}
		edges.clear();
	}

	int AddEdge(int from, int to, int cap, int cost)
	{
		edges.push_back(Edge(from, to, cap, 0, cost));
		edges.push_back(Edge(to, from, 0, 0, -cost));
		m = edges.size();
		G[from].push_back(m - 2);
		G[to].push_back(m - 1);
		return m - 2;
	}

	bool BellmanFord(int s, int t, int & cost)
	{
		for (int i = 0; i < n; ++i)
		{
			d[i] = INF;
		}
		memset(inq, 0, sizeof(inq));
		d[s] = 0;
		inq[s] = 1;
		p[s] = 0;
		a[s] = INF;
		queue<int> Q;
		Q.push(s);
		while (!Q.empty())
		{
			int u = Q.front();
			Q.pop();
			inq[u] = 0;
			for (int i = 0; i < G[u].size(); ++i)
			{
				Edge & e = edges[G[u][i]];
				if (e.cap > e.flow && d[e.to] > d[u] + e.cost)
				{
					d[e.to] = d[u] + e.cost;
					p[e.to] = G[u][i];
					a[e.to] = min(a[u], e.cap - e.flow);
					if (!inq[e.to])
					{
						Q.push(e.to);
						inq[e.to] = 1;
					}
				}
			}
		}
		if (d[t] == INF)
		{
			return false;
		}
		cost += d[t] * a[t];
		for (int u = t; u != s; u = edges[p[u]].from)
		{
			edges[p[u]].flow += a[t];
			edges[p[u] ^ 1].flow -= a[t];
		}
		return true;
	}

	int MincostMaxflow(int s, int t)
	{
		int cost = 0;
		while (BellmanFord(s, t, cost));
		return cost;
	}
};

int s[1000], t[1000], c[1000];
int id[1000];

int main()
{
	int n, k;
	scanf("%d %d", &n, &k);
	vector<int> v;
	for (int i = 0; i < n; ++i)
	{
		scanf("%d %d %d", &s[i], &t[i], &c[i]);
		v.push_back(s[i]);
		t[i] += s[i];
		v.push_back(t[i]);
	}
	sort(v.begin(), v.end());
	v.erase(unique(v.begin(), v.end()), v.end());
	MCMF mc;
	mc.init(v.size() + 2);
	int S = v.size();
	int T = v.size() + 1;
	for (int i = 0; i < n; ++i)
	{
		s[i] = lower_bound(v.begin(), v.end(), s[i]) - v.begin();
		t[i] = lower_bound(v.begin(), v.end(), t[i]) - v.begin();
		id[i] = mc.AddEdge(s[i], t[i], 1, -c[i]);
	}
	for (int i = 0; i < v.size() - 1; ++i)
	{
		mc.AddEdge(i, i + 1, k, 0);
	}
	mc.AddEdge(S, 0, k, 0);
	mc.AddEdge(v.size() - 1, T, k, 0);
	int ans = mc.MincostMaxflow(S, T);
	for (int i = 0; i < n; ++i)
	{
		printf("%d%s", mc.edges[id[i]].flow, i == n - 1 ? "\n" : " ");
	}
	return 0;
}
